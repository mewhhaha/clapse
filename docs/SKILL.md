...
- Dispatch now also applies root-kind + signature-family pruning before member checks: Not/And/Or roots route only on `ClassMethodExprTypeBool` + pure effect, compose roots only on compose-pure, map roots only on functor-pure, while Bool/Var remain empty and `Other` keeps full fallback.
