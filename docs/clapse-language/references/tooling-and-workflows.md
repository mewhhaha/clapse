...
```bash
deno run -A scripts/clap.mjs compile <input.clap> [output.wasm] [--entrypoint-export <name>] [--entrypoint-exports <csv>]
deno run -A scripts/clap.mjs compile-native <input.clap> [output.wasm] [--entrypoint-export <name>] [--entrypoint-exports <csv>]
deno run -A scripts/clap.mjs compile-native-debug <input.clap> [output.wasm] [artifacts-dir] [--entrypoint-export <name>] [--entrypoint-exports <csv>]
deno run -A scripts/clap.mjs compile-debug <input.clap> [output.wasm] [artifacts-dir] [--entrypoint-export <name>] [--entrypoint-exports <csv>]
deno run -A scripts/clap.mjs emit-wat <input.clap> [output.wat]
deno run -A scripts/clap.mjs format <file>
deno run -A scripts/clap.mjs format --write <file>
deno run -A scripts/clap.mjs format --stdin
deno run -A scripts/clap.mjs lsp --stdio
deno run -A scripts/clap.mjs bench [iterations]
```
...
- `emit-wat` is a kernel-owned pure emitter surface returning text output (`wat`)
  from Clap source-side logic; JS remains the I/O boundary for writing stdout/files.
...
- `compile`/`selfhost-artifacts`/`format`/`lsp` use compiler-wasm mode by
  ...
  - `compile_mode` now supports `native` / `kernel-native` for kernel-native
    ...
    Specifiers resolve through `clap.json` `include` paths (for bare
    specifiers) and filesystem-relative resolution for `./`, `../`, and `/`
    specifiers.
    Built-in prelude aliases (`"prelude"`, `"compiler/prelude"`)
    resolve to `lib/compiler/prelude.clap` without
    requiring `include`.
    ...
    payloads under `lib/compiler/native_compile*.clap`, so this exception is
    ...
    Legacy env
    toggles `CLAP_ENTRYPOINT_DCE` and `CLAP_INTERNAL_ENTRYPOINT_DCE` remain
  ...
  - host-bridge compile execution is removed from JS boundary code; compile
    requests must execute on a native clap compiler artifact.
  ...
  - `CLAP_COMPILE_ENGINE=native|kernel-native` remains accepted for explicit
    native intent on plain `compile`.
  - compiler wasm is resolved from `CLAP_COMPILER_WASM_PATH`, then
    `artifacts/latest|out/clap_compiler.wasm` searched from
  ...
  - bootstrap seed mode is opt-in:
    `CLAP_USE_WASM_BOOTSTRAP_SEED=1` routes compile requests in
    ...
    `CLAP_USE_WASM_BOOTSTRAP_SEED=1` is set.
    Compile-request auto-fallback has been removed from the ABI path. Use
    `CLAP_USE_WASM_BOOTSTRAP_SEED=1` explicitly only for non-`kernel-native`
    bootstrap-seed shaping.
    The helper CLI is
    `deno run -A scripts/ts-seed/run-bootstrap-seed.mjs --request '<json>' --seed-wasm <path>`.
  - `just clap-bin`/`just install` embed `artifacts/latest/clap_compiler.wasm`
    into `artifacts/bin/clap` when present, so formatter/LSP can run without
  ...
  - `just bootstrap-strict-native-seed` builds a strict native bootstrap seed at
    ...
    `CLAP_DISABLE_WASM_BOOTSTRAP_FALLBACK=1`) and raw
    ...
    `lib/compiler/native_compile.clap`, or
    `lib/compiler/native_compile_reachability.clap` are newer than the
    ...
    It no longer falls back to `artifacts/latest/clap_compiler.wasm`
    implicitly.
  - `just bootstrap-compiler` now also prefers
    `artifacts/strict-native/seed.wasm` before an existing
    `artifacts/latest/clap_compiler.wasm` when no explicit bootstrap path is
  ...
  - `just pre-tag-verify` now generates the strict-native seed first and defaults
    ...
    `CLAP_COMPILER_WASM_PATH` is not explicitly set, including
    `just native-strict-producer-check` with
    `CLAP_DISABLE_WASM_BOOTSTRAP_FALLBACK=1` and
    `just native-source-version-propagation-gate` in raw producer mode
    (`CLAP_DISABLE_WASM_BOOTSTRAP_FALLBACK=1`) so source-version transitivity
    ...
    (`CLAP_NATIVE_SELFHOST_PROBE_HOPS` to override).
    `scripts/native-selfhost-probe.mjs` supports `--hops <n>` for transitive
    closure checks (default `--hops 1`).
  - `just bootstrap-compiler` recompiles `lib/compiler/kernel.clap` into a compiler
    wasm artifact using a bootstrap compiler wasm (`CLAP_BOOTSTRAP_COMPILER_WASM_PATH`,
    then `CLAP_COMPILER_WASM_PATH`, then the existing output/latest compiler
    artifact, then `CLAP_BOOTSTRAP_STRICT_NATIVE_SEED_PATH`
    ...
    (`CLAP_DISABLE_WASM_BOOTSTRAP_FALLBACK=1 just native-strict-producer-check`) plus raw
  ...
  - if direct kernel self-compile output fails strict producer checks,
    ...
    defaults to `2` (`CLAP_BOOTSTRAP_NATIVE_SELFHOST_PROBE_HOPS` to override).
  - `just bootstrap-strict-native-seed` retain/build checks now default to
    probe depth `2` (`CLAP_STRICT_NATIVE_SEED_PROBE_HOPS` to override).
  - `just install` now runs `just bootstrap-strict-native-seed` and
    `just bootstrap-compiler`, then refreshes
    `artifacts/latest/clap_compiler.wasm` + `.d.ts` from that kernel recompile.
    If `deno compile` cannot run (for example offline `denort` download
    failures), install reuses an existing `artifacts/bin/clap` when present,
    otherwise generates a `deno run` shim at `artifacts/bin/clap`.
  ...
  - `just install` runs wildcard-demand gate only when
    `CLAP_RUN_WILDCARD_DEMAND_CHECK=1`.
  ...
  - kernel-path compile responses are validated strictly at the JS boundary:
    ...
    `clap_run`. Tiny kernel-compiler outputs are hard failures.
...
- `compile-debug` contract:
  ...
  - `just native-strict-producer-check [wasm=...] [hops=...]` runs
    ...
    Compile-request auto-fallback has been removed from the ABI path. Use
    `CLAP_USE_WASM_BOOTSTRAP_SEED=1` explicitly for bootstrap-seed compile
  ...
  - bootstrap and pre-tag flows now default to
    ...
    in raw producer mode (`CLAP_DISABLE_WASM_BOOTSTRAP_FALLBACK=1`) to fail
  ...
  - `just native-boundary-strict-seed-scan` scans local wasm artifacts (and
    sibling `../clap2/artifacts/releases` when present) and reports which
    compiler seeds, if any, satisfy strict compile + emit-wat contract checks.
    Set `CLAP_STRICT_NATIVE_REQUIRE_NO_BOUNDARY_FALLBACK=1` (or pass
    ...
    `CLAP_STRICT_NATIVE_KERNEL_SELFHOST_HOPS=<n>`) to require kernel
  ...
  - `just bootstrap-native-producer-seed [seed=...] [out=...] [meta=...] [depth=...] [source_version=...]`
    ...
    + emit-wat contracts with `CLAP_DISABLE_WASM_BOOTSTRAP_FALLBACK=1` before
    ...
    `CLAP_NATIVE_PRODUCER_SEED_DEPTH`.
  ...
  - `just native-strict-producer-check-no-fallback [wasm=...] [hops=...] [source_version=...]`
    ...
    `CLAP_DISABLE_WASM_BOOTSTRAP_FALLBACK=1` for fail-closed producer-only
    verification.
  - `just native-source-version-propagation-gate [wasm=...] [hops=...] [source_version=...]`
    ...
    `CLAP_NATIVE_SOURCE_VERSION_REQUIRED` is unset, the gate derives the
    ...
    contract token. Set `CLAP_NATIVE_SEED_META_PATH` to override the metadata
    lookup path.
...
`scripts/explorer.mjs`. The page is intentionally narrow: editable Clap
...
and local syntax highlighting for Clap and WAT. This replaces the old deployment-oriented
...
the current Clap wasm benchmark fixtures against optimized native Rust
baselines using the same argument stream and checksum logic. The harness fails
if the Rust and Clap checksums diverge. It also prints a
`wasm-boundary-only` row so the report separates plain JS↔Wasm call cost from
the generated Clap program cost, plus `*-net` rows that subtract that
boundary-only cost from each Clap case. It accepts `iterations`, `warmup`,
...
present, the same table also includes a `clap-wasmi` row for each case. The
...
- `examples/bench_wasm_http_request_parser.clap`

`just native-source-owned-optimization-benchmark-gate` runs
`scripts/source-optimizer-benchmark-gate.mjs`. It compiles the representative
optimization fixtures through the source-owned `compile_mode: "native-debug"`
path, requires compiler-returned `codegen_ir.txt` and
`optimizer_stats.json`, decompiles the emitted wasm to WAT, and verifies
...
- `just wildcard-demand-check` (kernel demand-order regression check)
  - validated from `scripts/wasm-behavior-fixture-map.json` with source-hash
    drift checks against `examples/wildcard_demand_behavior_regressions.clap`
...
- `just verify-compiler-fixpoint`
  - rebuilds the committed compiler through the bounded strict-seed path and
    requires byte-identical `clap_compiler.wasm` plus identical
    `clap_compiler.d.ts`
  - now hard-fails if the rebuilt compiler exceeds `CLAP_MAX_COMPILER_WASM_BYTES`
    or diverges from the committed compiler artifact

## LSP and Formatter

- Formatter is implemented in the Clap kernel and returned by `format`
...
- LSP reads `plugins` from `clap.json` and adds compiled plugin artifacts to
...
## Project Configuration (`clap.json`)

- LSP reads `clap.json` from the current file directory, then walks parent
...
- `include` is the only supported module-search key in `clap.json`.
...
- `plugins` is a list of plugin source directories. Each directory is recursively
  scanned for `*.clap` files, and each plugin source is compiled to a sibling
...
- `include` contains directory names. Bare quoted import specifiers are resolved
  by checking `<dir>/<specifier>` and `<dir>/<specifier>.clap`.
  Relative/absolute specifiers (`./`, `../`, `/`) resolve from the importing
  file path.

## Tree-sitter and Helix

- Grammar source: `tree-sitter-clap/`
