...
This is the main compiler gate. It rebuilds the committed compiler through the
bounded strict-seed path and requires byte-identical wasm plus identical `.d.ts`
output. Oversized compiler candidates are rejected before promotion.
...
# public compile-surface strategy census over deduplicated `(entry, export)` scenarios
deno run -A scripts/selfhost-compile-strategy-report.mjs --manifest examples/selfhost_behavior_corpus.json --out out/selfhost-compile-strategy-report.json
...
Compiler fixpoint guard:

```bash
just verify-compiler-fixpoint
```

This is the primary compiler integrity proof for the committed wasm compiler: if
the bounded rebuild diverges from the checked-in compiler artifact, the gate
fails.
...
- Acceptance: correctness, performance, and DX gates are green on the
  committed bounded compiler artifact.
- Integrity gate: `just verify-compiler-fixpoint` is green.
