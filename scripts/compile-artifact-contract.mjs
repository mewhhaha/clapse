...
function hasLegacyHeaderPrefix(text, label) {
  return typeof text === "string" &&
    text.startsWith(`(${label}) `);
}
...
export function assertStructuralArtifacts(
  ...
  assert(
    hasHeader(lowered, "lowered_ir") ||
      (allowLegacyHeaderPrefix && hasLegacyHeaderPrefix(lowered, "lowered_ir")),
    `${context}: lowered_ir.txt should use structural lowered_ir format`,
  );
  assert(
    hasHeader(collapsed, "collapsed_ir") ||
      (allowLegacyHeaderPrefix && hasLegacyHeaderPrefix(collapsed, "collapsed_ir")),
