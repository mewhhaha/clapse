...
function trimCommonIndent(lines) {
  ...
  return lines.map((line) => line.startsWith(" ".repeat(minIndent)) ? line.slice(minIndent) : line);
}

function collectIndentedMethodSpans(lines, blockStart, blockEndExclusive) {
  const spans = new Map();
  const ambiguous = new Set();
  const blockLines = lines.slice(blockStart + 1, blockEndExclusive);
  let minIndent = null;
  ...
  for (let i = blockStart + 1; i < blockEndExclusive;) {
    ...
    const functionName = parseIndentedFunctionDefinitionName(rawLine);
    if (functionName.length === 0) {
      i += 1;
      continue;
    }
    let endLine = blockEndExclusive;
    for (let j = i + 1; j < blockEndExclusive; j += 1) {
      const nextLine = lines[j];
      const nextTrimmed = stripLineComment(nextLine).trim();
    ...
    spans.set(functionName, {
      ...
      startLine: i,
      endLine,
      sourceLines: trimCommonIndent(lines.slice(i, endLine)),
    });
    i = endLine;
  }
  return { spans, ambiguous };
...
function parseModuleSourceInfo(sourceText, sourcePath) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (
      ...
      parseFunctionDefinitionName(rawLine).length === 0
    ) {
      if (trimmed.startsWith("class ") && /\|[^]*,\s*where$/u.test(trimmed)) {
        throw new Error(
          `class fundep tails reject trailing commas in ${sourcePath}:${i + 1}`,
        );
      }
  ...
  for (const [name, span] of instanceMethodSpans.entries()) {
...
function collectFunctionReferencesFromSpan(spanSource, localFunctionNames) {
  ...
  const qualifiedRefs = [];
  const qualifiedSeen = new Set();
  const unqualifiedRefs = new Set();
  const addQualifiedRef = (moduleName, symbol) => {
    const cleanModuleName = String(moduleName ?? "").trim();
    const cleanSymbol = normalizeFunctionName(symbol);
    if (cleanModuleName.length === 0 || !isFunctionName(cleanSymbol)) {
      return;
    }
    const key = `${cleanModuleName}.${cleanSymbol}`;
  ...
  for (const match of source.matchAll(QUALIFIED_CALL_RE)) {
    ...
    addQualifiedRef(moduleName, symbol);
  }
  for (const tokenMatch of source.matchAll(NAME_OR_OPERATOR_TOKEN_RE)) {
    const token = tokenMatch[0];
    if (token.includes(".")) {
      const qualifiedMatch = token.match(
        /^([A-Za-z_][A-Za-z0-9_']*)\.(.+)$/u,
      );
      if (qualifiedMatch) {
        const [, moduleName, symbol] = qualifiedMatch;
        addQualifiedRef(moduleName, symbol);
      }
      continue;
    }
...
export async function buildDemandDrivenCompileInput(
  ...
  for (const modulePath of moduleOrder) {
    ...
    let keptFunctionSpans = [...info.functionSpans.values()]
    ...
    for (const importEntry of info.importEntries ?? []) {
      ...
      const targetPath = String(importEntry?.resolvedPath ?? "");
      if (targetPath.length === 0) {
        const fallbackModuleName = String(importEntry?.moduleName ?? "").trim();
        if (fallbackModuleName.length > 0) {
          aliasMap.set(alias, fallbackModuleName);
        }
        continue;
      }
      const targetInfo = moduleInfos.get(targetPath);
      ...
      aliasMap.set(alias, String(targetInfo.moduleName ?? "").trim());
    }
    const exported = info.exportNames.filter((name) => roots.has(name));
    const syntheticKeptSpans = keptFunctionSpans.filter((span) =>
      prunableDeclSpans.some((declSpan) =>
        span.startLine >= declSpan.startLine && span.startLine < declSpan.endLine
      )
    );
    let dropIdx = 0;
    let pruneDeclIdx = 0;
    ...
    if (lines.length > 0) {
      const emittedSyntheticNames = new Set();
      for (const span of syntheticKeptSpans) {
        if (emittedSyntheticNames.has(span.name)) {
          continue;
        }
        emittedSyntheticNames.add(span.name);
        if (lines.length > 0 && lines[lines.length - 1].trim().length > 0) {
          lines.push("");
        }
        lines.push(...span.sourceLines);
      }
