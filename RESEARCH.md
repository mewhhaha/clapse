...
- Expression signatures used by class-law guards are now cached once per expression state during each rewrite pass, preventing redundant signature recomputation. This is an internal optimization; guard predicates, cost policy, dispatch mode, rewrite semantics, and policies are unchanged.
